...
var link2 = link + "teachereditattendance.php";
var link3 = link + "teacherupdateattendance.php";

class TeacherEditAttendanceStudent extends StatefulWidget {
  final String course;
  final String section;
  final DateTime date;
  final String teacherID;

  TeacherEditAttendanceStudent({
  ...
  @override
  _TeacherEditAttendanceStudentState createState() =>
      _TeacherEditAttendanceStudentState();
}

class _TeacherEditAttendanceStudentState
    extends State<TeacherEditAttendanceStudent> {
  // List<Map<String, dynamic>> studentData = List.generate(
  //   20,
  //       (index) => {
  //     'student_id': '21k-322${index + 1}',
  ...
  void initState() {
    ...
    //   }
    // });
    fetchAttendanceData(widget.teacherID, widget.course, widget.section, widget.date);
  }


  List<Map<String, dynamic>> studentData = [];

  Future<void> fetchAttendanceData(String teacherID, String courseId, String section, DateTime date) async {
    try {
      final response = await http.post(
        Uri.parse(link2),
        body: {'teacher_id': teacherID, 'course_id': courseId, 'section': section, 'date': date.toString()},
      );
